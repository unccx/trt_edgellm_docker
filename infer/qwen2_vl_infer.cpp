...
void printUsage(char const* programName)
{
    std::cerr << "Usage: " << programName
              << " --engineDir=<path> --multimodalEngineDir=<path> --inputFile=<path> --outputFile=<path> [--debug]"
              << std::endl;
}

bool parseArgs(InferArgs& args, int argc, char* argv[])
{
    static struct option options[] = {{"help", no_argument, nullptr, InferOptionId::HELP},
        {"inputFile", required_argument, nullptr, InferOptionId::INPUT_FILE},
        {"engineDir", required_argument, nullptr, InferOptionId::ENGINE_DIR},
        {"multimodalEngineDir", required_argument, nullptr, InferOptionId::MULTIMODAL_ENGINE_DIR},
    ...
    while ((opt = getopt_long(argc, argv, "", options, nullptr)) != -1)
    {
        switch (opt)
        {
        case InferOptionId::HELP: args.help = true; return true;
        case InferOptionId::INPUT_FILE: args.inputFile = optarg; break;
        case InferOptionId::ENGINE_DIR: args.engineDir = optarg; break;
        case InferOptionId::MULTIMODAL_ENGINE_DIR: args.multimodalEngineDir = optarg; break;
        case InferOptionId::OUTPUT_FILE: args.outputFile = optarg; break;
        case InferOptionId::DEBUG: args.debug = true; break;
        default: return false;
        }
    }

    if (args.inputFile.empty() || args.outputFile.empty() || args.engineDir.empty() || args.multimodalEngineDir.empty())
    {
        return false;
    }
...
std::vector<rt::LLMGenerationRequest> parseInputFile(std::filesystem::path const& inputFilePath)
{
    ...
    check::check(inputData.contains("requests") && inputData["requests"].is_array(), "'requests' must be an array");
    auto const& requestsArray = inputData["requests"];

    for (size_t startIdx = 0; startIdx < requestsArray.size(); startIdx += static_cast<size_t>(batchSize))
    {
        ...
        size_t const endIdx = std::min(startIdx + static_cast<size_t>(batchSize), requestsArray.size());
        for (size_t requestIdx = startIdx; requestIdx < endIdx; ++requestIdx)
        {
            ...
            for (auto const& messageJson : requestItem["messages"])
            {
                ...
                else if (contentJson.is_array())
                {
                    ...
                    {
                        check::check(contentItemJson.contains("type"), "Each content item must contain 'type'");
                        rt::Message::MessageContent content;
                        ...
                        else if (content.type == "image")
                        {
                            ...
                            check::check(image.buffer != nullptr, "Failed to load image: " + content.content);
                            request.imageBuffers.push_back(std::move(image));
                        }
                        else
                        {
                            throw std::runtime_error("Only text/image content are supported in infer/qwen2_vl_infer");
                        }
                        message.contents.push_back(std::move(content));
                    }
                }
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        runtime = std::make_unique<rt::LLMInferenceRuntime>(args.engineDir, args.multimodalEngineDir, loraWeightsMap, stream);
    }
    catch (std::exception const& e)
    {
    ...
    for (size_t requestIdx = 0; requestIdx < batchedRequests.size(); ++requestIdx)
    {
        ...
        bool const ok = runtime->handleRequest(batchedRequests[requestIdx], response, stream);
        if (!ok)
        {
            hasFailure = true;
        }
        ...
        for (size_t batchIdx = 0; batchIdx < batchedRequests[requestIdx].requests.size(); ++batchIdx)
        {
            ...
            responseJson["output_text"] = ok ? response.outputTexts[batchIdx]
    ...
    try
    {
        ...
        check::check(outputFile.is_open(), "Failed to open output file: " + args.outputFile);
